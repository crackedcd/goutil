...
func GetDateBeforeToday(days int) time.Time {
    return getTimeNowObj().AddDate(0, 0, days)
}

func GetDatetimeBeforeNow(seconds int) time.Time {
    return getTimeNowObj().Add(-time.Second * time.Duration(seconds))
}

// Valid time units are "ns", "us" (or "µs"), "ms", "s", "m", "h".
func GetDatetimeDuration(s string) time.Time {
    d, _ := time.ParseDuration(s)
    return getTimeNowObj().Add(d)
}
...
func GetCurrentTimeUnix() int64 {
    return getTimeNowObj().Unix()
}

func GetCurrentTimeUnixNano() int64 {
    return getTimeNowObj().UnixNano()
}

func getTimeNowObj() time.Time {
    return time.Now()
}